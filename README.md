...
1. **Scrape and clean the congressional record (local, with pool threading)**: Code can be found in [scrape_cr_pt.py](https://github.com/lsc4ss-s22/final-project-congress/blob/master/scrape_cr_pt.py).
...
The results from committee-based congressional networks also state that black information leaders, information leaders who are more reachable, or involved in more and larger congressional committees to be more effective. The results from issue-based congressional networks is similar to the results from committee-based congressional networks, with some differences regarding the effect of interaction between eigenvector centrality and betweenness centrality, as well as hispanic and betweenness centrality. Although the reasons behind the similarities and differences is unclear, it is evident from both congressional networks that an increase in participation in committees and discussion about most frequent issues can potentially improve the effectiveness of elected representatives.
